      ...
      <div class="lg:w-1/2">
        ...
        <div v-if="productData.rating" class="flex items-center gap-2 mb-4">
          ...
          <span class="text-gray-500 text-sm"
            >({{ productData.reviews }} Reviews)</span
          >
        </div>
        ...
        <div class="mb-6" v-if="productData.price">
          <div class="flex items-baseline gap-3 mb-1">
            <span class="text-2xl font-bold text-gray-900"
              >${{ productData.price.toLocaleString() }}</span
            >
            <span
        ...
        <!-- Size Helper -->
        <div
          class="mb-8 p-4 bg-gray-50 rounded-xl"
          v-if="productData.sizeHelperText"
        >
          <button class="flex items-center justify-between w-full group">
            <span class="text-gray-700 font-medium">{{
              productData.sizeHelperText
            }}</span>
            <span
              class="text-purple-600 font-semibold group-hover:text-purple-700 transition-colors"
            >
      ...
      <div class="bg-white rounded-2xl max-w-md w-full mx-auto">
        <div class="p-6">
            ...
            <div class="flex items-center space-x-4">
              ...
              <div>
                ...
                <p class="text-sm text-gray-600">
                  Size: <span class="font-semibold">{{ selectedSize }}</span>
                </p>
          ...
          <div class="space-y-4 mb-6">
            ...
            >
              <div
                class="flex items-center justify-center w-5 h-5 border-2 rounded-full mr-3"
                :class="[
                  selectedProductTypes.includes(option.id)
                    ? getOptionBorderClass(option.id)
                    : 'border-gray-300',
                  isOptionDisabled(option.id) ? 'border-gray-300' : ''
                ]"
              >
                <div
...
const productTypeOptions = [
  { id: "stl", name: "STL File", description: "3D printable file format", category: "digital" },
  {
    id: "cam",
    name: "CAM Product",
    description: "Computer-aided manufacturing",
    category: "digital"
  },
  {
    id: "rubber",
    name: "Rubber Mold",
    description: "Flexible molding solution",
    category: "physical"
  },
  {
    id: "casting",
    name: "Casting Model",
    description: "Metal casting pattern",
    category: "physical"
  },
];
...
const getOptionBorderClass = (typeId) => {
  const option = productTypeOptions.find(opt => opt.id === typeId);
  if (option?.category === "digital") return "border-blue-500";
  if (option?.category === "physical") return "border-orange-500";
  return "border-green-500";
};

const getOptionDotClass = (typeId) => {
  const option = productTypeOptions.find(opt => opt.id === typeId);
  if (option?.category === "digital") return "bg-blue-500";
  if (option?.category === "physical") return "bg-orange-500";
  return "bg-green-500";
};

const getProductTypeBadgeClass = (typeId) => {
  const option = productTypeOptions.find(opt => opt.id === typeId);
  if (option?.category === "digital") return "bg-blue-100 text-blue-800";
  if (option?.category === "physical") return "bg-orange-100 text-orange-800";
  return "bg-green-100 text-green-800";
};

const getProductTypeBadgeTextClass = (typeId) => {
  const option = productTypeOptions.find(opt => opt.id === typeId);
  if (option?.category === "digital") return "text-blue-600";
  if (option?.category === "physical") return "text-orange-600";
  return "text-green-600";
};

const isOptionDisabled = (typeId) => {
  const option = productTypeOptions.find(opt => opt.id === typeId);
  if (!option) return false;
  
  // If no selections yet, all options are enabled
  if (selectedProductTypes.value.length === 0) return false;
  
  // Check if trying to mix digital and physical products
...
const closeProductTypeModal = () => {
  ...
  // Reset selections when modal is closed without confirmation
...
const selectProductType = (typeId) => {
  ...
  if (index > -1) {
    ...
    // If selecting a physical product, clear all digital products
    ...
    // If selecting a digital product, clear all physical products
...
const handleProductTypeSelection = () => {
  ...
  if (hasPhysicalProducts.value) {
    // Physical products require enquiry
    openEnquiryForm();
  } else if (hasDigitalProducts.value) {
    // Digital products can be bought directly
...
const proceedToCheckout = () => {
  ...
  console.log(
    "Proceeding to checkout with product types:",
    selectedProductTypes.value
  );
};
